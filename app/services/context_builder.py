...
class ContextBuilder(object):
    ...
    def _build_intro_text(self, state):
        ...
        return (
            ...
            "председательствующего - судьи {судья тп ио bold},\n"
            "при секретаре {секретарь тп ио bold},\n"
            "с участием:\n"
    ...
    def _build_prosecution_opinion_text(self, state):
        ...
        if state.prosecutor_proposes_recovery:
            action = "взыскать с осуждённого"
        else:
            action = "освободить осуждённого от взыскания процессуальных издержек"
        ...
        return (
            ...
            "который {1}.".format(action, reaction)
        )
    ...
    def _build_recovery_reasoning_text(self, state, full_total):
        ...
        if mode == "recovery":
            ...
            return (
                "В соответствии с ч. 6 ст. 132 УПК РФ, в связи с состоянием здоровья и применением к "
                "{подсудимый дп ио} принудительной меры медицинского характера в виде принудительного лечения "
                "в медицинской организации, оказывающей психиатрическую помощь в стационарных условиях, "
                "специализированного типа, суд считает необходимым освободить его от взыскания "
