...
class ContextBuilder(object):
    ...
    def _build_prosecution_opinion_text(self, state):
        ...
        return (
            "Государственный обвинитель предлагал процессуальные издержки {0}, "
            "{{который пол}} {1}.".format(action, reaction)
        )
    ...
    def _build_recovery_reasoning_text(self, state, full_total):
        ...
        if mode == "recovery":
            ...
            return (
                "В соответствии с ч. 6 ст. 132 УПК РФ, в связи с возрастом, семейным положением, состоянием здоровья "
                "{подсудимый рп ио}, суд считает необходимым освободить {его/её пол} от взыскания процессуальных издержек "
                "в связи с имущественной несостоятельностью."
            )
            ...
            return (
                "Поскольку уголовное дело в отношении {подсудимый рп ио} по существу не рассмотрено, вопрос о "
                "возможности взыскания с {него/неё пол} процессуальных издержек рассмотрению не подлежит."
            )

        elif mode == "not_participate":
            return (
                "Поскольку {подсудимый ио} в судебном заседании не участвовал, защитник был назначен {ему/ей пол} "
                "по инициативе суда, мнение {подсудимый рп ио} о возможности взыскания с {него/неё пол} процессуальных "
                "издержек и {его/её пол} имущественное положение не выяснялось, осободить {его/её пол} от "
    ...
    def _build_recovery_result_text(self, state, full_total):
        ...
        # Эти режимы не должны заполнять маркер — абзац удалится целиком
        if mode in ("not_considered", "not_participate"):
            return ""
