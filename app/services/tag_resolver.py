...
class TagResolver(object):
    ...
    def resolve_tag(self, raw_tag, context):
        ...
        # Если получатель — адвокатский кабинет, то в склоняемых формах
        # (например, «адвокат {наименование получателя рп} Иванов И.И.»)
        # название не нужно — убираем его. Реквизитную форму без падежа не трогаем.
        if case_short and base_key in (
            "наименование получателя",
            "адвокат получатель",
            "получатель",
        ):
            if "кабинет" in value.lower():
                return ""
